...
class TestEmployeeView(TestCase):
    ...
    def test_can_add_employee(self):
        role = Role.objects.create(role_id="10", title="TesterRole", hourly_wage="10")
        login = self.client.login(username='AuthorizedTestUserName', password='password')
        response = self.client.post("/add-employee/add", {"employee_id": "10", "first_name": "test_title", "last_name": "test_title", "date_of_birth": "1995-01-02", "gender": "M", "role": str(role.role_id), "pps_number": "8374928R", "phone_number": "+353872839059", "email": "test@testing.com", "start_date": "2022-03-11"})
        self.assertTrue(login)
    ...
    def test_can_delete_employee(self):
        ...
        existing_employees = Employee.objects.filter(employee_id=employee.employee_id)
    ...
    def test_cannot_add_employee(self):
        role = Role.objects.create(role_id="10", title="TesterRole", hourly_wage="10")
        login = self.client.login(username='UnauthorizedTestUserName', password='password')
        response = self.client.post("/add-employee/add", {"employee_id": "10", "first_name": "test_title", "last_name": "test_title", "date_of_birth": "1995-01-02", "gender": "M", "role": str(role.role_id), "pps_number": "8374948R", "phone_number": "+353872539059", "email": "test@testing.com", "start_date": "2022-03-11"})
        existing_employees = Employee.objects.all()
        self.assertTrue(login)
        self.assertTemplateUsed(response, "unauthorized.html")
        # Proved unsuccessful in adding employee if unauthorized by checking length of existing employees
        self.assertEqual(len(existing_employees), 0)
    ...
    def test_cannot_delete_employee(self):
        ...
        self.assertTrue(login)
        self.assertTemplateUsed(response, "unauthorized.html")
        # Proved unsuccessful in deleting employee if unauthorized by checking length of existing employees
