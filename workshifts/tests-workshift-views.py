...
class TestworkshiftView(TestCase):
    ...
    def test_get_edit_workshift(self):
        ...
        workshift= Workshift.objects.create(workshift_id="1", employee_id=employee, start_date="2022-10-11", start_time="09:00", end_date="2022-03-12", end_time="17:00", role_id=role)
        login = self.client.login(username='AuthorizedTestUserName', password='password')
    ...
    def test_can_add_workshift(self):
        ...
        employee = Employee.objects.create(employee_id="1", first_name="test_title", last_name="test_title", date_of_birth="1995-01-02", gender="M", role=role, pps_number="8374928R", phone_number="+353872839059", email="test@testing.com", start_date="2022-03-11")
        login = self.client.login(username='AuthorizedTestUserName', password='password')
        response = self.client.post("/add-workshift/add", {"workshift_id": "123456789", "employee_id": str(employee.employee_id), "start_date": "2022-03-11", "start_time": "09:00", "end_date": "2022-03-12", "end_time": "17:00", "role_id": str(role.role_id), "created_on": "2022-03-10"})
        self.assertTrue(login)
    ...
    def test_can_delete_workshift(self):
        ...
        workshift= Workshift.objects.create(workshift_id="1", employee_id=employee, start_date="2022-10-11", start_time="09:00", end_date="2022-03-12", end_time="17:00", role_id=role)
        login = self.client.login(username='AuthorizedTestUserName', password='password')
        response = self.client.get(f"/delete-workshift/{workshift.workshift_id}")
        existing_workshifts = Workshift.objects.filter(workshift_id=workshift.workshift_id)
    ...
    def test_cannot_add_workshift(self):
        role = Role.objects.create(role_id="10", title="TesterRole", hourly_wage="10")
        login = self.client.login(username='UnauthorizedTestUserName', password='password')
        response = self.client.post("/add-workshift/add", {"workshift_id": "10", "first_name": "test_title", "last_name": "test_title", "date_of_birth": "1995-01-02", "gender": "M", "role": str(role.role_id), "pps_number": "8374948R", "phone_number": "+353872539059", "email": "test@testing.com", "start_date": "2022-03-11"})
        existing_workshifts = Workshift.objects.all()
        self.assertTrue(login)
        self.assertTemplateUsed(response, "unauthorized.html")
        # Proved unsuccessful in adding workshift if unauthorized by checking length of existing workshifts
        self.assertEqual(len(existing_workshifts), 0)
    ...
    def test_cannot_delete_workshift(self):
        ...
        self.assertTrue(login)
        self.assertTemplateUsed(response, "unauthorized.html")
        # Proved unsuccessful in deleting workshift if unauthorized by checking length of existing workshifts
